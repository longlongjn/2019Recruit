 ...
 * update 2017-03-15  by zcho fix 组图图片变形问题
 */
...
(function ($) {
    flux.slider = function (elem, opts) {
        ...
        this.images = new Array(); //总图片
        this.imageLoadedCount = 0; //加载
        this.currentImageIndex = 0; //当前图片
        this.nextImageIndex = 1; //下张图片
        this.playing = false;
        ...
        //动画结束callback(data:{currentImage:img})
        ...
        this.element.bind('fluxTransitionEnd', function (event, data) {
            ...
            // if(_this.isPlaying())
            // 	_this.start();
            //console.log('fluxTransitionEnd',data);
    ...
    flux.slider.prototype = {
        ...
        finishedLoading: function () {
            ...
            if (this.options.controls) {
                ...
                this.nextButton = $('<a href="#">&rsaquo;</a>').css(css).css3({
                    'border-radius': '4px'
                });
                if (this.options.nextButton !== '') {
                    //alt="后一个"
                    this.nextButton =
                        $('<a href="#"><img src="' + this.options.nextButton + '"  style="max-width:100px;max-height:100px;"/></a>')
                            .css(cssImg);
                }
                this.nextButton.appendTo(this.surface).bind('click', function (event) {
                    event.preventDefault();
                    _this.next();
                });
                ...
                this.prevButton = $('<a href="#">&lsaquo;</a>').css(css).css3({
                    'border-radius': '4px'
                });
                if (this.options.prevButton !== '') {
                    //alt="前一个"
                    this.prevButton =
                        $('<a href="#"><img src="' + this.options.prevButton + '"  style="max-width:100px;max-height:100px;"/></a>')
                            .css(cssImg);
                }
                this.prevButton.appendTo(this.surface).bind('click', function (event) {
                    event.preventDefault();
                    _this.prev();
                });
        ...
        //根据图片大小，计算offsetTop offsetLeft width height
        getImageBgSize: function (_img) {
            var obj = {};
            var nh = 0,
                nw = 0;

            nh = _img.naturalHeight / _img.naturalWidth * this.width;
            if (this.height > nh) {
                nw = _img.naturalWidth / _img.naturalHeight * this.height;
                obj.height = this.height;
                obj.width = nw;
                obj.offsetLeft = (nw - this.width) / 2;
                obj.offsetTop = 0;
            } else {
                obj.width = this.width;
                obj.height = nh;
                obj.offsetTop = (nh - this.height) / 2;
                obj.offsetLeft = 0;
            }
            return obj;
        },
        setupImages: function () {
            var img1 = this.getImage(this.currentImageIndex),
                _offsetBg = this.getImageBgSize(img1),
                css1 = {
                    // 'url("'+img1.src+'") 50% 50% / contain no-repeat #ffffff',#fff显示当前
                    // 'background': 'url("'+img1.src+'") center center / cover no-repeat',
                    // 'background': 'url("'+img1.src+'") left top / cover no-repeat',
                    'background-repeat': 'no-repeat',
                    'background-size': _offsetBg.width + 'px ' + _offsetBg.height + 'px',
                    'background-image': 'url("' + img1.src + '")',
                    'background-position': '-' + _offsetBg.offsetLeft + 'px -' + _offsetBg.offsetTop + 'px',
                    'z-index': 101,
                    'cursor': 'auto',
                    //cust background-position: top center;
                    // background-size: 100% 100% cover 铺满变形 or contain 不变形
                    //'background-repeat':'no-repeat',
            ...
            $(this.image1).attr('naturalWidth', img1.naturalWidth);
            $(this.image1).attr('naturalHeight', img1.naturalHeight);
            $(this.image1).attr('imageIndex', this.currentImageIndex);

            $(this.image1).attr('offsetTop', _offsetBg.offsetTop);
            $(this.image1).attr('offsetLeft', _offsetBg.offsetLeft);

            this.image1.css(css1).children().remove();

            var img2 = this.getImage(this.nextImageIndex);
            var _offset2 = this.getImageBgSize(img2);
            this.image2.css({
                // 'url("'+this.getImage(this.nextImageIndex).src+'") 50% 50% / contain no-repeat #ffffff',#fff显示当前
                // 'background': 'url("'+this.getImage(this.nextImageIndex).src+'") center center / cover no-repeat',
                // 'background': 'url("'+this.getImage(this.nextImageIndex).src+'") left top / cover no-repeat',
                'background-repeat': 'no-repeat',
                'background-size': _offset2.width + 'px ' + _offset2.height + 'px',
                'background-image': 'url("' + img2.src + '")',
                'background-position': '-' + _offset2.offsetLeft + 'px -' + _offset2.offsetTop + 'px',
                'z-index': 100,
                //cust
                //'background-repeat':'no-repeat',
                //'background-size':'contain',  //background-size: 100% 100% or contain
                //'background-position': 'top center',
                //'background-color':'#fff',
                // 'background-size':this.width + "px " + this.height + "px"
            }).show();
...
(function ($) {
    ...
    $.fn.css3 = function (props) {
        ...
        for (var prop in props) {
            // 各厂Add the vendor specific versions
            for (var i = 0; i < prefixes.length; i++)
                css['-' + prefixes[i] + '-' + prop] = props[prop];

            // 默认Add the actual version
            css[prop] = props[prop];
        }
    ...
    flux.transition = function (fluxslider, opts) {
        //console.log('transition construct',fluxslider,opts);
        this.options = $.extend({
            requires3d: false, //默认2d
            after: function () {
        ...
        run: function () {
            ...
            setTimeout(function () {
                if (_this.options.execute !== undefined) {
                    //thisthisthis,clear bgcolor for display next page
                    _this.slider.image1.css("background-color", "")
                    //console.log('execute call this',_this);
    ...
    flux.transition_grid = function (fluxslider, opts) {
        return new flux.transition(fluxslider, $.extend({
            ...
            setup: function () {
                ...
                var imgWidth = this.slider.width,
                    imgHeight = this.slider.height;
                ...
                for (var i = 0; i < this.options.columns; i++) {
                    ...
                    for (var j = 0; j < this.options.rows; j++) {
                        ...
                        var tile = $('<div class="flux-cube tile tile-' + i + '-' + j + '"></div>').css({
...
(function ($) {
    flux.transitions.bars = function (fluxslider, opts) {
        return new flux.transition_grid(fluxslider, $.extend({
            ...
            delayBetweenBars: 40,
            renderTile: function (elem, colIndex, rowIndex, colWidth, rowHeight, leftOffset, topOffset) {
                var _left = parseInt(this.slider.image1.attr("offsetLeft")),
                    _top = parseInt(this.slider.image1.attr("offsetTop"));
                $(elem).css({
                    ...
                    // "background-size": this.slider.width + "px ",
                    'background-position': '-' + (leftOffset + _left) + 'px -' + _top + 'px'
                }).css3({
                    'transition-duration': '800ms',
            ...
            execute: function () {
                ...
                var height = this.slider.image1.height(); //this.slider.height; //
...
(function ($) {
    flux.transitions.bars3d = function (fluxslider, opts) {
        return new flux.transition_grid(fluxslider, $.extend({
            ...
            renderTile: function (elem, colIndex, rowIndex, colWidth, rowHeight, leftOffset, topOffset) {
                // var _left = parseInt(this.slider.image1.attr("offsetLeft"));
                var _top = parseInt(this.slider.image1.attr("offsetTop"));
            ...
            execute: function () {
                ...
                // var naturalheight = parseInt(this.slider.image1.attr("naturalheight"));
                // var naturalwidth = parseInt(this.slider.image1.attr("naturalwidth"));

                var _this = this,
                    height = this.slider.height, //this.slider.image1.height();
    ...
    flux.transitions.zip = function (fluxslider, opts) {
        return new flux.transitions.bars(fluxslider, $.extend({
            execute: function () {
                ...
                var height = this.slider.height; //this.slider.image1.height();
    ...
    flux.transitions.blocks = function (fluxslider, opts) {
        return new flux.transition_grid(fluxslider, $.extend({
            ...
            renderTile: function (elem, colIndex, rowIndex, colWidth, rowHeight, leftOffset, topOffset) {
                ...
                $(elem).css({
                    //cust old 'background-image': this.slider.image1.css('background-image'),
                    'background': this.slider.image1.css('background'),
                    'background-position': '-' + (leftOffset + _left) + 'px -' + (topOffset + _top) + 'px',
                    //cust
                    // "background-size": this.slider.width + "px " + this.slider.height + "px",
                    'background-repeat': 'no-repeat'
                }).css3({
...
(function ($) {
    flux.transitions.concentric = function (fluxslider, opts) {
        return new flux.transition(fluxslider, $.extend({
            ...
            alternate: false,
            setup: function () {
                var w = this.slider.width, //this.slider.image1.width(),
                    h = this.slider.height, //this.slider.image1.height(),
                ...
                for (var i = 0; i < blockCount; i++) {
                    ...
                    var block = $('<div></div>').attr('class', 'block block-' + i).css({
                        ...
                        position: 'absolute',
                        // top: 0,//((h-thisBlockSize)/2) +'px',
                        // left: 0,//((w-thisBlockSize)/2)+'px',
                        ...
                        // "background-size":'640px 1008.46px',
...
(function ($) {
    flux.transitions.cube = function (fluxslider, opts) {
        return new flux.transition(fluxslider, $.extend({
            ...
            perspective: 1000,
            setup: function () {
                var width = this.slider.width; //this.slider.image1.width();
                var height = this.slider.height; //this.slider.image1.height();
                ...
                var currentFace = $('<div class="face current"></div>').css($.extend(css, {
                    //cust old background: this.slider.image1.css('background-image')
                    background: this.slider.image1.css('background'),
                    //cust new
                    // 'background-size': this.slider.width + 'px ' + this.slider.height + 'px',
                    'background-repeat': 'no-repeat'
                ...
                var nextFace = $('<div class="face next"></div>').css($.extend(css, {
                    background: this.slider.image2.css('background'), // background-image
                    //cust new
                    // 'background-size': this.slider.width + 'px ' + this.slider.height + 'px',
                    'background-repeat': 'no-repeat'
                })).css3({
...
(function ($) {
    flux.transitions.turn = function (fluxslider, opts) {
        return new flux.transition(fluxslider, $.extend({
            ...
            setup: function () {
                var _top = parseInt(this.slider.image1.attr("offsetTop"));
                var _top2 = parseInt(this.slider.image2.attr("offsetTop"));
                var _left = parseInt(this.slider.image1.attr("offsetLeft"));
                var _left2 = parseInt(this.slider.image2.attr("offsetLeft"));
                var tab = $('<div class="tab"></div>').css({
                    ...
                    front = $('<div></div>').appendTo(tab).css({
                        //cust old 'background-image': this.slider.image1.css('background-image'),
                        'background': this.slider.image1.css('background'),
                        'background-position': (this.options.direction == 'left' ? '-' + ((this.slider.image1.width() / 2) + _left) : '-' + _left) + 'px -' + _top + 'px',
                        width: '100%',
                        height: '100%',
                        position: 'absolute',
                        top: '0',
                    ...
                    back = $('<div></div>').appendTo(tab).css({
                        //cust old 'background-image': this.slider.image2.css('background-image'),
                        'background': this.slider.image2.css('background'),
                        'background-position': (this.options.direction == 'left' ? '-' + _left2 + 'px' : '-' + ((this.slider.image1.width() / 2) + _left2)) + 'px -' + _top2 + 'px',
                        width: '100%',
                        height: '100%',
                        position: 'absolute',
                        top: '0',
                        ...
                        transform: flux.browser.rotateY(180),
                        'backface-visibility': 'hidden',
                        'background-position-x': (this.options.direction == 'left' ? '-' + _left + 'px' : '-' + ((this.slider.image1.width() / 2) + _left)) + 'px'
                    }),
                    ...
                    current = $('<div></div>').css({
                        ...
                        //cust old 'background-image': this.slider.image1.css('background-image'),
                        'background': this.slider.image1.css('background'),
                        'background-position': (this.options.direction == 'left' ? '-' + _left + 'px' : '-' + ((this.slider.image1.width() / 2) + _left)) + 'px -' + _top + 'px',
                        'z-index': 100,
                        //cust
                        // 'background-size': this.slider.width + "px " + this.slider.height + "px",
                        // 'background-size': this.slider.width + "px ",
...
(function ($) {
    flux.transitions.slide = function (fluxslider, opts) {
        return new flux.transition(fluxslider, $.extend({
            ...
            setup: function () {
                ...
                var width = this.slider.width,
                    ...
                    currentImage = $('<div class="current"></div>').css({
                        ...
                        top: '0px',
                        left: '0px',
                        //cust old background-image
                        background: this.slider[this.options.direction == 'left' ? 'image1' : 'image2'].css('background'),
                        //cust background-position: top center
                        'background-repeat': 'no-repeat',
                    ...
                    nextImage = $('<div class="next"></div>').css({
                        ...
                        left: width + 'px',
                        //cust old background-image
                        background: this.slider[this.options.direction == 'left' ? 'image2' : 'image1'].css('background'),
                        //cust
                        'background-repeat': 'no-repeat',
                        // 'background-size':width+'px '+height+'px', //contain
                        // 'background-position':'left top' // top center
                    }).css3({
                        'backface-visibility': 'hidden'
                    });
            ...
            execute: function () {
                var _this = this,
                    delta = this.slider.width; // this.slider.image1.width()
